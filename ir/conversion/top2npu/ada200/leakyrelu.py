...
@_register_tranformation_rule(TransformRule.LEAKYRELU_LOWERING)
...
def _get_quant_para(input_scale,output_scale):
    ...
    def QuantizeMultiplierSmallerThanOneExp(x):
    ...
    def QuantizeMultiplierGreateThanOne(x):
        q, shift = np.frexp(x)
        q_fixed = np.round(q * (1<<31)).astype(np.int32)
...
def cmodel(x, input_shape, npu_leakyrelu):

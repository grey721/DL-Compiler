...
def get_pre_process_dict_without_conv(para_buf):
    num_tile = para_buf[0]
    ...
    pre_process_dict["para_stride"] = 0
    pre_process_dict["para_kernel_w"] = 0
    pre_process_dict["para_stride_mux"] = 0

    pre_process_dict["para_kernel_h"] = 0
    pre_process_dict["para_kernel_cmod"] = 0

    pre_process_dict["para_fmi_c"] = fmi_c
    pre_process_dict["para_fmi_scmod"] = 0
    ...
    pre_process_dict["para_pad_fmi_wscmod"] = 0

    pre_process_dict["para_pad_zero_point"] = pad_zero_point

    pre_process_dict["para_fmo_w"] = fmi_w

    pre_process_dict["para_fmo_h"] = fmi_h
    pre_process_dict["para_num_win"] = 0
    pre_process_dict["para_num_psum"] = 0
    pre_process_dict["para_kernel_hwcmod"] = 0
...
if __name__ == "__main__":
    ...
    conv_type = 0
    base = 0
    fmi_raddr_b0 = addr0 + base
    fmi_raddr_b1 = addr1 + base
    fmi_raddr_b2 = addr2 + base
    fmi_raddr_b3 = addr3 + base
    fmi_h = 28
    fmi_w = 208
    fmi_c = 16
    fmi_coffset = 1
    ...
    process_param = pre_process_param(pre_process_dict)
    ...
    path = '/data/xieshuhan/tinynpu/test/model/yolov3/1/block1/pre_param.txt'
    pre_writeTxt(res_txt, path)
    # pass
